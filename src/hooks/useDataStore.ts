...
export const useDataStore = () => {
  ...
  const updateData = useCallback((newLinks: LinkItem[], newCategories: Category[]) => {
    ...
    localStorage.setItem(
      LOCAL_STORAGE_KEY,
      JSON.stringify({ links: newLinks, categories: newCategories }),
    );
  }, []);
  ...
  const updateLink = useCallback(
    (data: Omit<LinkItem, 'createdAt'>) => {
      ...
      const updated = links.map((l) =>
        l.id === data.id ? { ...l, ...data, url: processedUrl } : l,
      );
  ...
  const deleteCategories = useCallback(
    (catIds: string[]) => {
      ...
      const fallbackCategory = newCats.find((c) => c.id === 'common') || newCats[0];
      const newLinks = links.map((l) =>
        toDelete.has(l.categoryId) ? { ...l, categoryId: fallbackCategory.id } : l,
      );
      updateData(newLinks, newCats);
    },
    [links, categories, updateData, notify],
