...
export function useSyncEngine(options: UseSyncEngineOptions = {}): UseSyncEngineReturn {
  ...
  const pushToCloud = useCallback(
    async (data: Omit<YNavSyncData, 'meta'>, force: boolean = false): Promise<boolean> => {
      setSyncStatus('syncing');
  ...
  const createBackup = useCallback(
    async (data: Omit<YNavSyncData, 'meta'>): Promise<boolean> => {
      setSyncStatus('syncing');
  ...
  const restoreBackup = useCallback(
    async (backupKey: string): Promise<YNavSyncData | null> => {
      setSyncStatus('syncing');
